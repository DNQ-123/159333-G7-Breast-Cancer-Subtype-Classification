# -*- coding: utf-8 -*-
"""
Fast mRMR + RandomForest for PAM50 subtyping
- 5-fold CV（与 XGBoost 版一致）
- 60 iterations RandomizedSearchCV（可按需调整）
- 搜索结束把树数加到 1000 再在全训练集拟合一次
- 结果保存格式与 XGBoost 分支完全一致
"""
...
class MRMRSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        # relevance: 互信息
        self.mi_ = mutual_info_classif(X, y, random_state=self.random_state, discrete_features=False)
...
# --------------------
# 4. Pipeline（RF 分类器；去掉不必要的缩放层以对齐 XGB 版）
# --------------------
pipe = Pipeline([
...
random_search = RandomizedSearchCV(
    ...
    n_iter=60,                         # 你原 RF 版是 60 次，保留
    scoring='balanced_accuracy',       # 与 XGB 版一致
    cv=cv,
    verbose=2,
    n_jobs=-1,
    random_state=RANDOM_STATE
...
# --------------------
# 5.5. 详细的 5 折交叉验证评估（保存每折报告与混淆矩阵）
# --------------------
best_model = random_search.best_estimator_

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
results_dir = f"results_mrmr_rf_{timestamp}"
...
for fold_idx, (train_idx, val_idx) in enumerate(cv.split(X_train, y_train), 1):
    ...
    plt.title(f'Fold {fold_idx} Confusion Matrix (RF)')
    plt.ylabel('Actual')
...
# --------------------
# 6. 把树数加到 1000，全训练集再拟合（与 XGB 版一致）
# --------------------
...
plt.title('Final Test Set Confusion Matrix (mRMR + Random Forest)')
plt.ylabel('Actual')
plt.xlabel('Predicted')
plt.tight_layout()
...
plt.title('Confusion Matrix (mRMR + RF, fast)')
plt.ylabel('Actual'); plt.xlabel('Predicted')
plt.tight_layout()
plt.savefig('confusion_matrix_mrmr_rf_fast.png', dpi=220, bbox_inches='tight')
plt.close()

# --------------------
# 8. 保存所有结果文件（与 XGB 版完全对齐）
# --------------------
...
# 8.2 CV摘要
cv_summary_content = f"""5-Fold Cross-Validation Summary - mRMR + Random Forest
========================================
Architecture: mRMR Feature Selection + Random Forest Classifier
========================================
...
# 8.4 RF 分支信息 JSON（键值对齐 XGB 版）
rf = best_model.named_steps['clf']
imp_series = pd.Series(rf.feature_importances_, index=selected_genes).sort_values(ascending=False)

rf_info = {
    "model_type": "mRMR + RandomForest",
...
# 8.7 模型文件（结果目录 + 根目录）
joblib.dump(best_model, os.path.join(results_dir, 'pam50_rf_mrmr_model.pkl'))
joblib.dump(best_model, 'pam50_rf_mrmr_fast.pkl')

# 8.8 复现 Bundle（结果目录 + 根目录）
bundle_data = {
    "model": best_model,
...
joblib.dump(bundle_data, os.path.join(results_dir, "rf_mrmr_complete_bundle.pkl"))
joblib.dump(
    {"model": best_model, "genes_order": list(gene_cols),
     "train_median": train_median, "label_names": list(label_names)},
    "rf_mrmr_fast_bundle.pkl"
)
...
print(f"  - rf_branch_info.json: RF详细信息")
print(f"  - final_training_metrics.csv: 训练指标")
print(f"  - pam50_rf_mrmr_model.pkl: 训练好的模型")
print(f'\nSelected {len(selected_genes)} genes')
print(f'Root directory files: pam50_rf_mrmr_fast.pkl, rf_mrmr_fast_bundle.pkl, etc.')
